//+------------------------------------------------------------------+
//| SolunaAI_Example_EA.mq5 — AI Analysis Dashboard Example         |
...
void DrawPanel()
{
   ...
   PutLbl("SAI_lv_entry_val", PanelX+55, PanelY+240, DoubleToString(g_aiEntry, _Digits), 8, clrWhite);
   PutLbl("SAI_lv_sl_lbl", PanelX+130, PanelY+240, "SL:", 8, clrGray);
   PutLbl("SAI_lv_sl_val", PanelX+148, PanelY+240, DoubleToString(g_aiSL, _Digits), 8, clrCrimson);
   PutLbl("SAI_lv_tp_lbl", PanelX+20, PanelY+255, "TP:", 8, clrGray);
   PutLbl("SAI_lv_tp_val", PanelX+38, PanelY+255, DoubleToString(g_aiTP, _Digits), 8, clrLimeGreen);
...
string WrapTxt(string text, int maxLen)
{
   ...
   while(pos < len)
   {
      ...
      for(int i = pos + cut; i > pos; i--)
         if(StringGetCharacter(text, i) == ' ') { sp = i; break; }
...
void DeleteAllObjects(string prefix)
{
   for(int i = ObjectsTotal(0) - 1; i >= 0; i--)
   {
      ...
      if(StringSubstr(n, 0, StringLen(prefix)) == prefix) ObjectDelete(0, n);
   }
}
